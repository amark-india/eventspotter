...
The live nature of event detection makes the extraction process further complex. This results in a partial lack of benchmark from which a canonical natural language chain can be started. In this work we explore the plausibility of using a mixture of various information extraction and semantic web techniques for event detection and demonstrate that it is possible to obtain an increase in recall even with a linear combination of linguistic and knowledge based approaches grounded on a live stream of social event data in order to spot and disambiguate event entities in plain text. Both the source code of the experiments and the benchmark proposed in this work are publicly available at \url{http://github.com/amark-india/eventspotter}.
...
As baselines we initially chose OpenCalais (as an off-the-shelf solution) and Stanford NER (version 3.2.0) properly trained with the data from the training set. 
A long survey on off-the-shelf systems that are able to detect musical events landed us in an extremely specific domain, where systems are tailored for performing reasonably well on tailored data. This consideration is further proven by the results achieved and it explains why OpenCalais is not able at all to detect events of the same kind available in the Eventmedia dataset. For this reason, we chose to omit the OpenCalais results from this paper.
